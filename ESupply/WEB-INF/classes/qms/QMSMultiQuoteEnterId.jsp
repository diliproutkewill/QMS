<%--
	...
	esupply - v 1.x.
	Program	Name	: QMSMultiQuoteEnterId.jsp
	Product Name	: QMS
	Module Name		: 
...
<%
   ...
   System.out.println("quoteStatus-------"+request.getParameter("quoteStatus"));
   System.out.println("quoteActive-------"+request.getParameter("quoteActive"));
...
function popUpWindow(input)
{
	...
	var status="";
	var active="";
	var multiQuoteFilter = " and  is_multi_quote = 'Y' "; // Added by kishore For Single QuoteFilter
	if(operation=='Modify')
	{
...
//	Url		="qms/ListOfValues.jsp?lovid=QUOTE_MASTER&tabArray="+tabArray+"&formArray="+formArray+"&lovWhere="+lovWhere+"&operation=QUOTEMODIFY&search= where QUOTE_ID LIKE '"+document.forms[0].QuoteId.value+"~'"+companyname+" "+orginlocation+" "+destlocation+" "+mode+" ";
		Url		="qms/ListOfValues.jsp?lovid=QUOTE_MASTER&tabArray="+tabArray+"&formArray="+formArray+"&lovWhere="+lovWhere+"&operation=QUOTEMODIFY&search="+encodeURIComponent(" where QUOTE_ID LIKE '"+document.forms[0].QuoteId.value+"~'"+multiQuoteFilter+companyname+" "+orginlocation+" "+destlocation+" "+mode+" "); // Modified by kishore For Single QuoteFilter
//@@ Ended for 215432
	}
		...
		if(document.forms[0].Status.value!="")
	{
		...
		Url		="qms/ListOfValues.jsp?lovid=QUOTE_MASTER_VIEW&tabArray="+tabArray+"&formArray="+formArray+"&lovWhere="+lovWhere+"&operation=QUOTECOPY&search="+encodeURIComponent(" where QUOTE_ID LIKE '"+document.forms[0].QuoteId.value+"~' "+multiQuoteFilter+companyname+" "+orginlocation+" "+destlocation+" "+status+" "+mode+" "+active+""); // Modified by kishore For Single QuoteFilter
	}
	else
	{
		if(document.forms[0].Status.value!="")
	...
	//@@ Modified by subrahmanyam for the wpbn id: 215432 on 24/08/10
	Url		="qms/ListOfValues.jsp?lovid=QUOTE_MASTER_VIEW&tabArray="+tabArray+"&formArray="+formArray+"&lovWhere="+lovWhere+"&operation=QUOTEVIEW&search= "+encodeURIComponent(" where QUOTE_ID LIKE '"+document.forms[0].QuoteId.value+"~' "+multiQuoteFilter+companyname+" "+orginlocation+" "+destlocation+" "+status+" "+mode+" "+active+"");	// Modified by kishore For Single QuoteFilter
	}
...
function checkAjaxValidMultiQuote()
	{
	...
	if(value !='')
		{
           ...
           var url='';
		  	url='<%=request.getContextPath()%>/QMSQuoteAjaxController?type=AJAX&SearchOption=validQuote&multiQuote=Y&id='+value;
...
<body bgcolor="#FFFFFF">
<form   method="post" action="QMSMultiQuoteController" onsubmit='return validate()'>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr bgcolor="#FFFFFF" valign="top"> 
        <td>
         <table width="100%" cellpadding="4" cellspacing="0">
			<tr valign="top" class='formlabel'>
			   <td  width="903">Multi Quote - <%=display%> </td><!--modified by silpa.p on 1-06-11-->
...
<table width="100%" cellpadding="4" cellspacing="1">
			...
			<tr class='formdata'>
				...
				<tr class='formdata'>
				<td colspan="1">
					<input class="text" maxLength="20" name="QuoteId" id="quoteid" value='<%=quoteId!=null?quoteId:""%>'  size="15" onBlur="this.value=this.value.toUpperCase();chr(this); checkAjaxValidMultiQuote();"><!--   // Added By Kishroe Podili for the Multi Quote Validation Ajax Call  -->
