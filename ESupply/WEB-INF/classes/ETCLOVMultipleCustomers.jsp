...
%
	...
	Program Name	:ETCLOVMultipleCustomers.jsp
	Module Name		:ETrans
	Task			:ETCLOVMultipleCustomers
	Sub Task		:ETCLOVMultipleCustomers
	Author Name		:
	Date Started	:
	Date Completed	:
	Date Modified	:
	Description		:This file is invoked when clicked on the Location LOV in the LocationEnterId Screen. In this all the Location 

-->

<%@ page import="javax.naming.InitialContext,com.foursoft.esupply.common.util.Logger,
				java.util.ArrayList,
...
<%!
	private static final String FILE_NAME	=	"ETCLOVMultipleCustomers.jsp";
%>
...
<%
    ...
    if(listHandler == null)
    {
		...
		try
		    ...
		    customerIds				=	remote.getCustomerIds(loginbean);
	...
	if(customerIds!=null)
	{
		listHandler   = new LOVListHandler(customerIds,FoursoftConfig.NOOFRECORDSPERPAGE,requiredAttributes);
	...
	{
		Logger.error(FILE_NAME,"ETCLOVMultipleCustomers.jsp : " , e.toString());
	}
	}
	...
	String 		fileName	= "ETCLOVMultipleCustomers.jsp";  
    ...
    {
      Logger.error(FILE_NAME,"ETCLOVMultipleCustomers.jsp : " +ex);
    }
...
function submitForm(var1)
{
	...
	document.forms[0].action="ETCLOVMultipleCustomers.jsp?pageNo="+var1;
...
%>
		document.forms[0].customerIds.options[ <%= i %> ] = new Option('<%= str %>','<%=str%>','<%= str %>');
<%
	}//end of for loop
...
function setCustomerIds()
{
	...
	{
		alert("Please select atleast one customerId")
	}
	else
	{
...
%>
		var len		=<%=fromWhat%>;
		window.opener.showCuctomerIds(document.forms[0].listTypes1,'<%=wheretoset%>');
		var cond 	='<%= ArrayOrNot%>' ;
		resetValues();
	}
<%
...
<body class='formdata' onUnLoad='toKillSession()' onbeforeunload='selectallValues();' onLoad="populateCustomerIds()" onKeyPress='onEscKey()'>
<form method="post" action="">
<input type=hidden name='pageNo' value='<%=pageNo%>'>

<center>
<b><font face="Verdana">Customer Ids</font></b>
</center>
<br>
<%
	if(currentPageList.size() > 0)
...
<center>
<select size="10" name="customerIds" MULTIPLE onKeyPress='onEnterKey()' class="select">
</select>
...
<input type="button" value='<<' name="left" class='input' onClick='moveDestSelectedRecords(document.forms[0].listTypes1,document.forms[0].customerIds)'>
<select size="10" name="listTypes1" MULTIPLE onDblClick='setCustomerIds()' onKeyPress='onEnterKey()' class="select">
</select>
</center>
<br>
<center>
...
<br><br>
<center>
<textarea cols=30 class='select' rows=6 readOnly >No CustomerIds available.
</textarea>
<br><br>
