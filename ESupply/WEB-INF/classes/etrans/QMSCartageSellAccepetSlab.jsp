...
function Mandatory()
{
	...
	document.forms[0].action="QMSCartageController?Operation=cartageAccept&subOperation=updateSellRate&index="+'<%=index%>'+"&serchStr=etValue&weightBreak=Slab";
...
//@@Ended by subrahmanyam for the Enhancement 170759
			...
			if(document.forms[0].slabMargin.value.length==0||document.forms[0].slabMargin.value==0)
			{
				alert('Please Enter the Slab Margin');
				document.forms[0].slabMargin.focus();
				return false;
			}
			if(document.forms[0].maxMargin.value.length==0||document.forms[0].maxMargin.value==0)
			{
				for(var j=0;j<marginValues.length;j++)
				{
				//	if(document.getElementsByName(slabBreak+i+j)[0].value=='MAX' && document.getElementsByName(slabValue+j)[0].value!='')
		//	modified  for issue 172884 by VLAKSHMI on 8/06/2009
      	if(document.getElementsByName(slabBreak+j)[0].value=='MAX' && document.getElementsByName(slabValue+j)[0].value!='')
        	{
...
%>			    
                <td><%=hMap.get("Ind"+slabList.get(j))!=null?hMap.get("Ind"+slabList.get(j)):""%>&nbsp;<%=!"".equalsIgnoreCase(chargeRate)?chargeRate:"-"%></td>
...
%>		
		<tr valign="top" class='formlabel'>
		<!--   <td colspan="20">Margin Calculations</td> --><!-- @@Commented by subrahmanyam for Enhancement 170759 on 02/06/09 -->
            <td colspan="22">Margin Calculations</td><!-- @@Ended by subrahmanyam for Enhancement 170759 on 02/06/09 -->
		</tr>
        <tr valign="top" class='formdata'>
        ...
        <div id='absolute' style="DISPLAY:none">
			<table width="100%" cellpadding="4" cellspacing="1">
			 ...
			 <td width='26%'></td>
<!--@@Added by subrahmanyam for the Enhancement 170759 on 02/06/09  -->
				<td align='left'>Base Margin<font color='red'>* </font>
					<input type='text' class='text' name="baseMargin" size="5" value='<%=sellDOB.getBaseMargin()%>'  maxlength='6' onblur='getKeyCode(this);return chrnum(this)'>
				</td>

<!--@@Ended by subrahmanyam for the Enhancement 170759 on 02/06/09  -->

				<td align='left'>Min Margin<font color='red'>* </font>
					<input type='text' class='text' name="minMargin" size="5" value='<%=sellDOB.getMinMargin()%>'  maxlength='6' onblur='getKeyCode(this);return chrnum(this)'>
				</td>
				<td align='center'>Slab Margin<font color='red'>* </font>
					<input type='text' class='text' name="slabMargin" size="5" value='<%=sellDOB.getMargin()%>'  maxlength='6' onblur='getKeyCode(this);return chrnum(this)'>
				</td>
				<td align='right'>Max Margin<font color='red'>* </font>
					<input type='text' class='text' name="maxMargin" size="5" value='<%=sellDOB.getMaxMargin()%>' maxlength='6' onblur='getKeyCode(this);return chrnum(this)'>
				</td>
			 </tr>
			</table>
		</div>
...
<%
			...
			{
%>				<tr class='formdata'>
			 <td width='36%'><b><%="Delivery".equals(chargeType)?"DELIVERY MARGIN":"PICKUP MARGIN"%></b><font color='red'>* </font></td>
<%
				for(int i=0;i<slabList.size();i++)
				{
