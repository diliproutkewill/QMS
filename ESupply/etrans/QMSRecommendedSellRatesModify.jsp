...
<%
    logger  = Logger.getLogger(FILE_NAME);	
		...
		String			weighttBkValues_temp			=	null;//@@ Added by govind for the issue 268840
		int					noofRecords				=	0;
		int					noofPages				=	0;
...
%>
  ...
  <%
				...
				weighttBkValues_temp =  weighttBkValues[m];//@@ Added by govind for the issue 268840
					 if(weighttBkValues[m]!=null&&weighttBkValues[m]!="" && (diffDesc.equalsIgnoreCase(ratedesc[m]) ))
					  { 
						...
						if("2".equals(shipmentMode)&&"FCL".equals(consoleTpe))
						  {
								weighttBkValues_temp = weighttBkValues[m].length() > 4?weighttBkValues_temp.substring(0,4):weighttBkValues_temp;
						  }
							else	
							weighttBkValues_temp = weighttBkValues[m].length() > 5?weighttBkValues_temp.substring(5,weighttBkValues_temp.length()):weighttBkValues_temp;
						 %> <td><%=weighttBkValues_temp%></td>
<%			
