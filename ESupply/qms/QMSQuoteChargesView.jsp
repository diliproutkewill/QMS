...
<%
  ...
  String      str1[]              = null;
	...
	try
	{
		...
		//@@Added by kiran.v on 05/08/2011 for Wpbn Issue-	256087
		quoteDate			  =  masterDOB.getCreatedDate();
					...
					<td>Min</td>
                     <%tempDesc=chargeInfo.getRateDescription();}
					  ...
					  <%}
						...
						{%>
					   <td><font color="<%=color%>">
					  <%if("List".equalsIgnoreCase(masterDOB.getWeightBreak())|| (chargeInfo.getBasis()!= null && !"Per Container".equals(chargeInfo.getBasis()))){%>
...
%>					<!-- <td><font color="<%=color%>"><%=df.format(new Double(chargeInfo.getRecOrConSellRrate()))%></font></td> --><!-- commented for 180161 -->
			...
			<td><font color="<%=color%>"><%=round(chargeInfo.getRecOrConSellRrate())%></font></td><!-- ended for 180161 -->
				    ...
				    || (isNumber(chargeInfo.getBreakPoint()) && "SLAB".equalsIgnoreCase(chargeInfo.getWeight_break()) && "FLAT".equalsIgnoreCase(chargeInfo.getRate_Type()) ))
					  {
					  %>
					   <td>Per Shipment</td>
					   <%}
					  else if("FSKG".equalsIgnoreCase(chargeInfo.getBreakPoint())||"SSKG".equalsIgnoreCase(chargeInfo.getBreakPoint()) || ((chargeInfo.getBreakPoint().toUpperCase().endsWith("FLAT")) &&	 !"Sea".equalsIgnoreCase(shipmentMode))
						  || (isNumber(chargeInfo.getBreakPoint()) && !"SLAB".equalsIgnoreCase(chargeInfo.getWeight_break()) && !"FLAT".equalsIgnoreCase(chargeInfo.getRate_Type()) ))
					  {%>
					  <td>Per Kilogram</td>
					   <%}
