...
esupply - v 1.x.
Program	Name		: QMSMultiQuoteChargesView.jsp
Product Name		: QMS
Module Name			: Quote
Task				: Adding/Modify
Date started		:
...
<%!
  ...
  private static final String FILE_NAME	=	"QMSMultiQuoteChargesView.jsp"; %>
<%
...
if(!"charges".equalsIgnoreCase(masterDOB.getQuoteWith()))
	...
	chargeInfoList = ((MultiQuoteFreightLegSellRates)charges.get(0)).getFreightChargesList();
					...
					totbreaks = chargeInfo.getMultiBreakPoints()!=null?chargeInfo.getMultiBreakPoints().length:0;
...
function calaculateSellRate(obj,info,bre,breaks,brValue,rsrValue){
		...
		if(bre==2){//@@ if margin ADD
            marginvalue = obj.value;
			if(marginvalue != '' && marginvalue.length >0 ){
			marginType	=	 document.getElementById("marginTypes"+info+(bre-1)+breaks).value;
			if(marginType	==	'A')
			 calSellRate	= parseFloat(brValue) + parseFloat(marginvalue);
			else if(marginType	==	'P')
			 calSellRate	= parseFloat(brValue) + (( parseFloat(brValue) * parseFloat(marginvalue) )/100);
			}
			
		}
		if(bre==5){//@@ if discount (subtract)

			discountvalue = obj.value;
			if(discountvalue != '' && discountvalue.length >0 ){
				discountType	=	 document.getElementById("discountTypes"+info+(bre-1)+breaks).value;
				if(discountType	==	'A')
			 calSellRate	= parseFloat(rsrValue) - parseFloat(discountvalue);
			else if(discountType	==	'P')
			 calSellRate	= parseFloat(rsrValue) - (( parseFloat(rsrValue) *parseFloat(discountvalue))/100);
			}
   ...
   function setAllValues(){
	<%
		legCharges	   = (MultiQuoteFreightLegSellRates)charges.get(0);
				...
				chargeInfo			= (MultiQuoteChargeInfo)freightCharges.get(0);

						if("on".equalsIgnoreCase(chargeInfo.getCarrierChecked())){%>
		// alert('<%=(masterDOB.getCarrierChecked())%>');
			  document.getElementById('selectCarrier').checked	= true;
		 <%}if("on".equalsIgnoreCase(chargeInfo.getFrequencyChecked())){%>
				    document.getElementById('selectFrequecy').checked	= true;
			 <%}if("on".equalsIgnoreCase(chargeInfo.getTransitTimeChecked())){%>
				  document.getElementById('selecttime').checked	= true;
			 <%}if("on".equalsIgnoreCase(chargeInfo.getRateValidityChecked())){%>
				  document.getElementById('selectvalidity').checked	= true;
					<%}%>
...
<BODY onLoad="initialize();setAllValuesOnLoad();roundAll();setSelectedIndexValueOnLoad();setAllValues()">
	...
	<td>
		<table width="100%" cellpadding="0" cellspacing="0">
		  ...
		  <%if(!"Charges".equalsIgnoreCase(masterDOB.getQuoteWith())){//Added by Anil.k
			...
			<td	colspan="2"><table cellpadding="4" cellspacing="1" width="100%"><tr class='formlabel'><td align='left'><%=shipmentMode%> FREIGHT QUOTE (<%=quoteName%>,<%=quoteStatus%>,<%=completeFlag%>)- <%=operation!=null?operation.toUpperCase():""%></td><td align=right><!-- Screen Id --></td>
			<%chargeInfoList = ((MultiQuoteFreightLegSellRates)charges.get(0)).getFreightChargesList();
					chargeInfo	=	(MultiQuoteChargeInfo)chargeInfoList.get(0);
...
%>
       <table width="100%" cellpadding="4" cellspacing="1" bgcolor='#FFFFFF'>
		<tr class="formdata">
             <!-- <td colspan="<%=colspan%>" align=center><%=headerDOB.getTypeOfService()!=null?headerDOB.getTypeOfService().toUpperCase():""%> </td>--><!--modified by silpa.p on 25-05-11-->
...
%>
			  ...
			  <td>Select<br>
			  <input type="checkbox" name='select<%=fr%>' disabled></td>
			  <td>Charge Name</td>
			  <td>Defined By</td>
			  <td>Breakpoint</td>
			  <td>Currency</td>
				...
				for(int j=0;j<originChargesSize;j++)
				{
					...
					if(originIndices!=null )
					{
						for(int i=0;i<originIndices.length;i++)
						{
							...
							checkedFlag				=	false;
                    	if(OriginChargesSelectedIndices!=null && OriginChargesSelectedIndices.length>0){//Modified by Anil.k
...
<%		            for(int k=0;k<originChargesInfoSize;k++)
                    ...
                    //  if("checked".equalsIgnoreCase(originCheckedFlag))	{
						  
					  %>
				
					  <tr class="formdata" cellpadding="4" align='center'>
					  <td><%if(k==0){%><input type='checkbox' name='originChkBox<%=fr%>' onclick="setValue(<%=j%>,'origin',<%=fr%>)" <%=originCheckedFlag%> disabled>
...
%>	<!-- @@ Commented by subrahmanyam for 180161 -->
				...
				<!-- @@ Added by subrahmanyam for 180161 -->
					  ...
					  <td>
					  <select class="select"  name="originMarginType<%=fr%><%=j%>" size="1"  onfocus='this.defaultIndex=this.selectedIndex;' onchange='this.selectedIndex=this.defaultIndex;' >
				...
				<%if("Yes".equalsIgnoreCase(MarginTest) || "<<Back".equalsIgnoreCase(submit)||"Y".equalsIgnoreCase(chargesDOB.getSelectedFlag())){
					%>
						<td align='center'><input class="text" type="text" value="<%=chargeInfo.getMargin()%>" size=5 name='originMargin<%=fr%><%=j%>'  <%=originMarginDisabled%>  onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','originDiscount<%=fr%>',<%=j%>,'Discount')" maxlength='10' readonly>

				<%}else{
					session.setAttribute("MarginTestCount",MarginTestCount);%>
					<td align='center'><input class="text" readonly type="text" value="<%=chargeInfo.getMargin()<0?0:chargeInfo.getMargin()%>" size=5 name='originMargin<%=fr%><%=j%>'  <%=originMarginDisabled%>  onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','originDiscount<%=fr%>',<%=j%>,'Discount')" maxlength='10'>
					  </td>
				<%}%>
					  <%}
%>
					...
					<%if("B".equalsIgnoreCase(chargesDOB.getSellBuyFlag())||"BC".equalsIgnoreCase(chargesDOB.getSellBuyFlag()))
						{%>
					 <td>
					  <select class="select"  name="originDiscountType<%=fr%><%=j%>"  onfocus='this.defaultIndex=this.selectedIndex;' onchange='this.selectedIndex=this.defaultIndex;'  size="1" disabled>
					  ...
					  <%}
						...
						{%>
						<td>
					  <select class="select"  name="originDiscountType<%=fr%><%=j%>"  size="1" id='noRound'  onfocus='this.defaultIndex=this.selectedIndex;' onchange='this.selectedIndex=this.defaultIndex;'>
						  <option value="A" <%="A".equals(chargeInfo.getDiscountType())?"selected":""%>>Absolute</option>
						  <option value="P" <%="P".equals(chargeInfo.getDiscountType())?"selected":""%>>Percent</option>
						</select>
					  </td>
					  <%}%>
					  <td><input class="text" type="text" readonly value="<%=df1.format(new Double(chargeInfo.getDiscount()))%>" size=5 name='originDiscount<%=fr%><%=j%>'  onchange="enableDisableMargin(this,'<%=chargeInfo.getDiscount()%>','originMargin<%=fr%>',<%=j%>,'Margin')" maxlength='10'></td>
					  
					  <%if("B".equalsIgnoreCase(chargesDOB.getSellBuyFlag()) || "S".equalsIgnoreCase(chargesDOB.getSellBuyFlag())){%>
					  <td><input class="text" readonly type="text" id='chargeSellrate' value="<%=round(chargeInfo.getSellRate())%>" size=5 readOnly name='originSellRate<%=fr%><%=j%>'></td>
					  <%}else{%>
					  <td><input class="text" type="text" readonly id='chargeSellrate' value="<%=df1.format(new Double(chargeInfo.getSellRate()))%>" size=5 readOnly name='originSellRate<%=fr%><%=j%>'></td>
					  <%}%>
					  <td><font color="<%=color%>"><%=chargeInfo.getBasis()!=null?chargeInfo.getBasis():""%></font></td>
					  <td><input class="text" readonly type="text" value="<%=chargeInfo.getRatio()!=null?chargeInfo.getRatio():""%>" size=5 readOnly>&nbsp;</td>
...
<%
			...
			<tr class="formheader" cellpadding="4" align='center'>
			 ...
			 <!--@@ modified by kiran.v on 08/08/2011 for Wpbn Issue 258778 -->
			  <td >Carrier   <br>       
			  <input type="checkbox"  name="selectCarrier" disabled></td> 
			  <td>Service Level <br> <input type="checkbox" name="selectService" disabled></td>
			  <td>Frequency   <br>   <input type="checkbox"  name="selectFrequecy" disabled></td>
			  <td>Approx Transit Time  <br> <input type="checkbox"  name="selecttime"  disabled></td>
			  <td>Freight Validity <br><input type="checkbox" name="selectvalidity" disabled></td>
			  <td>Currency</td>
			  <td>BR/RSR</td>
<%chargeInfoList = ((MultiQuoteFreightLegSellRates)charges.get(0)).getFreightChargesList();
					int infoSize	=	chargeInfoList.size();
						...
						//	if(chargeInfo.getChecked_Flag()!= null && !"-".equals(chargeInfo.getChecked_Flag().split(",")[brkCount]) && !"".equals(chargeInfo.getChecked_Flag().split(",")[brkCount]) && selectedIndexList== null)
                                                 if(chargeInfo.getChecked_Flag()!= null && !"-".equals(chargeInfo.getChecked_Flag().split(",")[brkCount]) && !"".equals(chargeInfo.getChecked_Flag().split(",")[brkCount]) && "Y".equals(chargeInfo.getChecked_Flag().split(",")[brkCount]) && selectedIndexList== null)
								checked_Flag = "checked";	
						%>  <td nowrap>
						    <%  //System.out.println("chargeInfo.getSellBuyFlag()=="+chargeInfo.getSellBuyFlag());
		...
		<%}else if(!"CSR".equalsIgnoreCase(chargeInfo.getSellBuyFlag())){
		//	System.out.println("******************"+s+"---------"+chargeInfo.getMultiRateDescriptions()[brkCount]);%>
        <%=	(!"A FREIGHT RATE".equalsIgnoreCase(chargeInfo.getMultiRateDescriptions()[brkCount])) ?s.substring(0,s.length()-2):s%>
         ...
         <%}%><br>
							...
							<%=selectedIndexList!= null?(selectedIndexList.contains(Integer.toString(brkCount))?"checked":""):checked_Flag%> disabled>
...
%>						   
	    <input type='hidden' name='marginTestFailed' value='<%=( chargeInfo.isMarginTestFailed())?"Y":"N"%>'/>
						   ...
						   <font color='<%=freightcolor%>'><%=chargeInfo.getOriginPort()%></font></td>
                           <td rowspan="8"><font color='<%=freightcolor%>'><%=chargeInfo.getDestPort()%></font></td>
						   <td rowspan="8"><font color='<%=freightcolor%>'><%=chargeInfo.getIncoTerms()!= null?chargeInfo.getIncoTerms():""%></font></td>
						   <td rowspan="8"><font color='<%=freightcolor%>'><%=chargeInfo.getCarrier()!= null?chargeInfo.getCarrier():""%></font></td>
						   <td rowspan="8"><font color='<%=freightcolor%>'><%=chargeInfo.getServiceLevel()!= null?chargeInfo.getServiceLevel():""%></font></td>
						   <td rowspan="8"><font color='<%=freightcolor%>'><%=chargeInfo.getFrequency()!= null?chargeInfo.getFrequency():""%></font></td>
						   <td rowspan="8"><font color='<%=freightcolor%>'><%=chargeInfo.getTransitTime()%></font></td>
						   <td nowrap rowspan="8"><font color='<%=freightcolor%>'><%=chargeInfo.getValidUpto()!=null?eSupplyDateUtility.getDisplayStringArray(chargeInfo.getValidUpto())[0]:""%></font></td>
						   <td rowspan="8"><font color='<%=freightcolor%>'><%=chargeInfo.getCurrency()%></font></td> 
						   <%for (int bre=0;bre<7;bre++){
							...
							<%}else if(bre==1 && !"RSR".equals(chargeInfo.getSellBuyFlag())){
							...
							<%}else if(bre==3 && !"SBR".equals(chargeInfo.getSellBuyFlag()) && !"BR".equals(chargeInfo.getSellBuyFlag())){
							  tempData	=	chargeInfo.getMultiSellRates();
										%>
								<td><b><font size='0.50' color='<%=freightcolor%>'>RSR</font></b></td>
										<%}else if(bre==4 && !"SBR".equals(chargeInfo.getSellBuyFlag()) && !"BR".equals(chargeInfo.getSellBuyFlag())) {
							  tempData	=	chargeInfo.getMultiDiscountTypes();
										%>
								<td><b><font size='0.50'>DiscountTypes</font></b></td>
										<%}else if(bre==5 && !"SBR".equals(chargeInfo.getSellBuyFlag()) && !"BR".equals(chargeInfo.getSellBuyFlag())){
                            ...
                            <%
								...
								String  disabled="";
								if(bre==1 || (bre==4 && !"SBR".equals(chargeInfo.getSellBuyFlag()) && !"BR".equals(chargeInfo.getSellBuyFlag()))){
						...
						id = '<%=bre==1?"marginTypes":"discountTypes"%><%=info%><%=bre%><%=breaks%>'  
						 onfocus='this.defaultIndex=this.selectedIndex;' onchange='this.selectedIndex=this.defaultIndex;' >
						  <option value="A" <%=tempData!=null?(( "A".equals(tempData[breaks]) || "-".equals(tempData[breaks]) && tempData[breaks] != null)?"selected":""):"selected"%>>Absolute</option>
						  <option value="P" <%=tempData!=null?("P".equals(tempData[breaks])?"selected":""):"selected"%>>Percent</option>
							</select>
								</td>

									<%}else if(bre==2 || (bre==5 && !"SBR".equals(chargeInfo.getSellBuyFlag()) && !"BR".equals(chargeInfo.getSellBuyFlag()))){
									...
									<td><input class='text' readonly size='2' type='text' name='<%=bre==2?"MarginVal":"DiscountVal"%><%=info%>'
									...
									sellRate	=	chargeInfo.getMultiSellRates()[breaks];
											...
											%>
									<td><input class='text' readonly  size='5' type='text' name='<%=bre==2?"MarginVal":"DiscountVal"%><%=info%>'
									id='<%=bre==2?"MarginVal":"DiscountVal"%><%=info%><%=bre%><%=breaks%>'
									value='<%=tempData!=null?tempData[breaks]:"0.0"%>' <%=tempData!=null&&"-".equals(tempData[breaks])?readOnly:""%>  /></td>

										<%}}else if(bre==6){
											if(tempData!=null && "-".equals(tempData[breaks]) ){%>
									<td><input class='text' readonly size='2' type='text' name='calsellrate<%=info%>'  value='-' readonly ></td>
											<%}else{%>
									<td><input class='text' readonly size='5' type='text' name='calsellrate<%=info%>' id='calsellrate<%=info%><%=bre%><%=breaks%>' value='<%=tempData!=null?tempData[breaks]!= null?tempData[breaks]:"0.00":"0.00"%>' readonly ></td>
										<%}}else if(bre==0 || (bre==3  && !"SBR".equals(chargeInfo.getSellBuyFlag()) && !"BR".equals(chargeInfo.getSellBuyFlag()))){%>
											<td><font color='<%=selectedIndexList!= null?(selectedIndexList.contains(Integer.toString(breaks))?(chargeInfo.isMarginTestFailed()?"#FF0000":""):""):""%> '><%=tempData[breaks]%></font></td>
									<%}else{%><td></td>
...
%>					  <tr class="formdata" cellpadding="4" align='center'>
					  <td><%if(k==0){%><input type='checkbox' name='destChkBox<%=fr%>' onclick="setValue(<%=j%>,'dest',<%=fr%>)" <%=destCheckedFlag%> disabled>
...
%>			  		<!-- @@ Commented by subrahmanyam for 180161 -->
				...
				<!-- @@Ended by subrahmanyam for 180161 -->
					<td>
					  <select class="select"  name="destMarginType<%=fr%><%=j%>" size="1"   onfocus='this.defaultIndex=this.selectedIndex;' onchange='this.selectedIndex=this.defaultIndex;' >
						  <option value="A" <%="A".equals(chargeInfo.getMarginType())?"selected":""%>>Absolute</option>
						  <option value="P" <%="P".equals(chargeInfo.getMarginType())?"selected":""%>>Percent</option>
					  </select>
					  </td>
					...
					//	int mTCount4=MarginTestCount1.intValue();
								...
								<td align='center'><font color="<%=color%>"><input class="text" type="text" readonly value="<%=chargeInfo.getMargin()%>" size=5 name='destMargin<%=fr%><%=j%>'  onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','destDiscount<%=fr%>',<%=j%>,'Discount')" <%=destMarginDisabled%> maxlength='10'></font></td>

					  <%}else{
								session.setAttribute("MarginTestCount",MarginTestCount);%>
								<td align='center'><font color="<%=color%>"><input class="text" type="text"  readonly value="<%=chargeInfo.getMargin()<0?0:chargeInfo.getMargin()%>" size=5 name='destMargin<%=fr%><%=j%>'  onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','destDiscount<%=fr%>',<%=j%>,'Discount')" <%=destMarginDisabled%> maxlength='10'></font></td>
					  <%}%>
...
%>						<td><font color="<%=color%>"><%=df.format(new Double(chargeInfo.getRecOrConSellRrate()))%></font></td>
						...
						{%>
						<td>
						<select class="select"  name="destDiscountType<%=fr%><%=j%>" size="1" disabled  onfocus='this.defaultIndex=this.selectedIndex;' onchange='this.selectedIndex=this.defaultIndex;'>
						  <option value="A" <%="A".equals(chargeInfo.getDiscountType())?"selected":""%>>Absolute</option>
						  <option value="P" <%="P".equals(chargeInfo.getDiscountType())?"selected":""%>>Percent</option>
						</select>
					  </td>
					  <%}
						...
						{	%>
                          <td>
						<select class="select"  name="destDiscountType<%=fr%><%=j%>" size="1"  onfocus='this.defaultIndex=this.selectedIndex;' onchange='this.selectedIndex=this.defaultIndex;' >
						  <option value="A" <%="A".equals(chargeInfo.getDiscountType())?"selected":""%>>Absolute</option>
						  <option value="P" <%="P".equals(chargeInfo.getDiscountType())?"selected":""%>>Percent</option>
						</select>
					  </td>
						<%}%>
					  <td><font color="<%=color%>"><input class="text" type="text" readonly value="<%=df1.format(new Double(chargeInfo.getDiscount()))%>" size=5 name='destDiscount<%=fr%><%=j%>'
					    onchange="enableDisableMargin(this,'<%=chargeInfo.getDiscount()%>','destMargin<%=fr%>',<%=j%>,'Margin')" maxlength='10'></font></td>
					  ...
					  <!-- Added for 180161 -->
					  <%if("B".equalsIgnoreCase(chargesDOB.getSellBuyFlag()) || "S".equalsIgnoreCase(chargesDOB.getSellBuyFlag())){%>
					  <td><font color="<%=color%>"><input class="text" type="text" id='Destrates' readonly  value="<%=round(chargeInfo.getSellRate())%>" size=5 readOnly name='destSellRate<%=fr%><%=j%>'></font></td>
					  <%}else{%>
					  <td><font color="<%=color%>"><input class="text" type="text" readonly value="<%=df1.format(new Double(chargeInfo.getSellRate()))%>" size=5 readOnly name='destSellRate<%=fr%><%=j%>'></font></td>
					  <%}%>
					  <!-- Ended for 180161 -->
					  <td><font color="<%=color%>"><%=chargeInfo.getBasis()!=null?chargeInfo.getBasis():""%></font></td>
					  <td><font color="<%=color%>"><input class="text" type="text"  readonly  value="<%=chargeInfo.getRatio()!=null?chargeInfo.getRatio():""%>" size=5 readOnly></font></td>
					  <input type='hidden' name='marginTestFailed' value='<%=(checkedFlag && chargeInfo.isMarginTestFailed())?"Y":"N"%>'/>
					  <td colspan ="<%=tempCount%>"></td>
					</tr>
<%
...
%>
					 <tr class="formheader" cellpadding="4" align='center'>
			  <td>Select<br>
			  <input type="checkbox" name='select<%=fr%>' onclick='selectAll(<%=fr%>)' disabled></td>
			  <td>Charge Name</td>
			  <td>Defined By</td>
			  <td>Breakpoint</td>
			  <td>Currency</td>
				...
				for(int j=0;j<originChargesSize;j++)
				{
					...
					/*if(originIndices!=null )
					{
						for(int i=0;i<originIndices.length;i++)
						{
							...
							checkedFlag				=	false;
                    	if(OriginChargesSelectedIndices.length>0){
							if(originIndices[i]==j&&"Y".equalsIgnoreCase(OriginChargesSelectedIndices[i]))
							{
								checkedFlag = true;
								break;
...
%>	<!-- @@ Commented by subrahmanyam for 180161 -->
				...
				<%if("Yes".equalsIgnoreCase(MarginTest) || "<<Back".equalsIgnoreCase(submit)||"Y".equalsIgnoreCase(chargesDOB.getSelectedFlag())){
					%>
						<td align='center'><input class="text" type="text" readonly value="<%=chargeInfo.getMargin()%>" size=5 name='originMargin<%=fr%><%=j%>'  <%=originMarginDisabled%>  onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','originDiscount<%=fr%>',<%=j%>,'Discount')" maxlength='10'>

				<%}else{
					session.setAttribute("MarginTestCount",MarginTestCount);%>
					<td align='center'><input class="text" readonly type="text" readonly value="<%=chargeInfo.getMargin()<0?0:chargeInfo.getMargin()%>" size=5 name='originMargin<%=fr%><%=j%>'  <%=originMarginDisabled%>  onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','originDiscount<%=fr%>',<%=j%>,'Discount')" maxlength='10'>
					  </td>
				<%}%>
					  <%}
%>
					...
					<%if("B".equalsIgnoreCase(chargesDOB.getSellBuyFlag())||"BC".equalsIgnoreCase(chargesDOB.getSellBuyFlag()))
						{%>
					 <td>
					  <select class="select"  name="originDiscountType<%=fr%><%=j%>"  size="1" disabled  onfocus='this.defaultIndex=this.selectedIndex;' onchange='this.selectedIndex=this.defaultIndex;'>
						  <option value="A" <%="A".equals(chargeInfo.getDiscountType())?"selected":""%>>Absolute</option>
						  <option value="P" <%="P".equals(chargeInfo.getDiscountType())?"selected":""%>>Percent</option>
						</select>
					  </td>
					  ...
					  <%if("B".equalsIgnoreCase(chargesDOB.getSellBuyFlag()) || "S".equalsIgnoreCase(chargesDOB.getSellBuyFlag())){%>
					  <td><input class="text" type="text" id='chargeSellrate' readonly value="<%=round(chargeInfo.getSellRate())%>" size=5 readOnly name='originSellRate<%=fr%><%=j%>'></td>
					  <%}else{%>
					  <td><input class="text" type="text" id='chargeSellrate' readonly value="<%=df1.format(new Double(chargeInfo.getSellRate()))%>" size=5 readOnly name='originSellRate<%=fr%><%=j%>'></td>
					  <%}%>
					  <td><font color="<%=color%>"><%=chargeInfo.getBasis()!=null?chargeInfo.getBasis():""%></font></td>
					  <td><input class="text" type="text" readonly value="<%=chargeInfo.getRatio()!=null?chargeInfo.getRatio():""%>" size=5 readOnly>&nbsp;</td>
...
%>			  		<!-- @@ Commented by subrahmanyam for 180161 -->
				...
				<!-- @@Ended by subrahmanyam for 180161 -->
					...
					//	int mTCount4=MarginTestCount1.intValue();
								...
								<td align='center'><font color="<%=color%>"><input class="text" type="text" value="<%=chargeInfo.getMargin()%>" size=5 readonly name='destMargin<%=fr%><%=j%>'  onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','destDiscount<%=fr%>',<%=j%>,'Discount')" <%=destMarginDisabled%> maxlength='10'></font></td>

					  <%}else{
								session.setAttribute("MarginTestCount",MarginTestCount);%>
								<td align='center'><font color="<%=color%>"><input class="text" readonly type="text" value="<%=chargeInfo.getMargin()<0?0:chargeInfo.getMargin()%>" size=5 name='destMargin<%=fr%><%=j%>'  onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','destDiscount<%=fr%>',<%=j%>,'Discount')" <%=destMarginDisabled%> maxlength='10'></font></td>
					  <%}%>
<!-- @@ Ended by subrahmanyam for the Enhancement 154381 on 03/02/09 -->
<%					}
%>						<td><font color="<%=color%>"><%=df.format(new Double(chargeInfo.getRecOrConSellRrate()))%></font></td>
					  ...
					  <%}
						...
						<%}%>
					  <td><font color="<%=color%>"><input class="text" readonly type="text" value="<%=df1.format(new Double(chargeInfo.getDiscount()))%>" size=5 name='destDiscount<%=fr%><%=j%>'
					   onchange="enableDisableMargin(this,'<%=chargeInfo.getDiscount()%>','destMargin<%=fr%>',<%=j%>,'Margin')" maxlength='10'></font></td>
					  <!-- <td><font color="<%=color%>"><input class="text" type="text" value="<%=df1.format(new Double(chargeInfo.getSellRate()))%>" size=5 readOnly name='destSellRate<%=j%>'></font></td> -->
					  <!-- Added for 180161 -->
					  <%if("B".equalsIgnoreCase(chargesDOB.getSellBuyFlag()) || "S".equalsIgnoreCase(chargesDOB.getSellBuyFlag())){%>
					  <td><font color="<%=color%>"><input class="text"  type="text" id='Destrates'  readonly value="<%=round(chargeInfo.getSellRate())%>" size=5 readOnly name='destSellRate<%=fr%><%=j%>'></font></td>
					  <%}else{%>
					  <td><font color="<%=color%>"><input class="text" type="text" value="<%=df1.format(new Double(chargeInfo.getSellRate()))%>" size=5 readOnly name='destSellRate<%=fr%><%=j%>'></font></td>
					  <%}%>
					  <!-- Ended for 180161 -->
					  <td><font color="<%=color%>"><%=chargeInfo.getBasis()!=null?chargeInfo.getBasis():""%></font></td>
...
%>
				 <input class="input" name="submit" type="submit" value="<<Back" onclick='setSubmitValue(this.value)'>
				  ...
				  <input type="hidden" name="marginTestFlag" value='F'>	 
