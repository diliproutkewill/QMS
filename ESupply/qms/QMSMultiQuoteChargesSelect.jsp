...
<%
 long start = System.currentTimeMillis();
   ...
   MultiQuoteFreightLegSellRates legRateDetails		   = null;
	...
	//Commented and Modified by Rakesh on 24-03-2011
	//String		 colspan				  = '';	  
	int				 colspan				  = 0;	
...
//@@ Ended by subrahmanyam for the Enhancement 154381 on 09/02/09
	...
	try
	{
          selectedIndexList = (String)session.getAttribute("selectedIndexList")!= null?","+(String)session.getAttribute("selectedIndexList"):(String)session.getAttribute("selectedIndexList");
		...
		selectedIndexList     = selectedIndexList == null ? ","+finalDOB.getMultiQuoteSelectedBreaks():selectedIndexList;
	...
	if(!isNaN(obj.value))
{
		...
		if(bre==2){//@@ if margin ADD
			...
			if(marginType	==	'A')
			 calSellRate	= round(parseFloat(brValue) + parseFloat(obj.value ));
			else if(marginType	==	'P')
			 calSellRate	= round(parseFloat(brValue) + (( parseFloat(brValue) * parseFloat(obj.value ) )/100));
       ...
       //    alert("330--rsr---"+rsrValue+"--br--"+brValue)
			...
			if(discountType	==	'A')
			 calSellRate	= round(parseFloat(rsrValue) - parseFloat(obj.value));
			
			else if(discountType	==	'P')
			 calSellRate	= round(parseFloat(rsrValue) - (( parseFloat(rsrValue) * parseFloat(obj.value) )/100));
...
// @@ modified by kiran.v on 08/08/2011 for Wpbn Issue 258778 
 function setAllValues(){
	// alert('<%=subOperation%>');
	<%
		if("SELLRATES".equalsIgnoreCase(subOperation))	{
		legCharges	   = (MultiQuoteFreightLegSellRates)charges.get(0);
				...
				if("on".equalsIgnoreCase(chargeInfo.getCarrierChecked())){%>
							document.getElementById('selectCarrier').value="checked";
			               document.getElementById('carrier').checked	= true;
		    <%}if("on".equalsIgnoreCase(chargeInfo.getFrequencyChecked())){%>
				 document.getElementById('selectFrequency').value="checked";
				    document.getElementById('frequency').checked	= true;
			 <%}if("on".equalsIgnoreCase(chargeInfo.getTransitTimeChecked())){%>
				    document.getElementById('selectTransitTime').value="checked";
				  document.getElementById('apprTranistTime').checked	= true;
			 <%}if("on".equalsIgnoreCase(chargeInfo.getRateValidityChecked())){%>
				   document.getElementById('selectFrieghtValidity').value="checked";
				  document.getElementById('freightValidity').checked	= true;
					<%}%>
				 <%if("on".equalsIgnoreCase(chargeInfo.getServiceChecked())){%>
				  document.getElementById('selectService').value="checked";
				 document.getElementById('serviceLevel').checked	= true;
					<%}}%>
   }
...
function setSelectedIndexValueOnLoad() 
	...
	<%}if("checked".equals(request.getParameter("selectTransitTime"))){%>
			document.forms[0].apprTranistTime.checked=true;
			document.forms[0].selectTransitTime.value="checked";
	<%}if("checked".equals(request.getParameter("selectFrieghtValidity"))){%>
			document.forms[0].freightValidity.checked=true;
			document.forms[0].selectFrieghtValidity.value="checked";
	<%}%>
...
{	
	...
	}	   //Added by Rakesh on 24-03-2011
	else if(obj.value=="T")
	{
		if(obj.checked)
			document.forms[0].selectTransitTime.value="checked";
		else
			document.forms[0].selectTransitTime.value="unchecked";
}
	else if(obj.value=="V")
	{
		if(obj.checked)
			...
			document.forms[0].selectFrieghtValidity.value="unchecked";
	}	   //Ended by Rakesh on 24-03-2011
...
function setSelectedIndexValue(obj,breakcount)
	...
	}else
	{
	   selectedlist = document.forms[0].selectedIndexList.value;
	    //alert(selectedlist+"--------"+breakcount+",")
	   //alert(selectedlist.replace(breakcount+",",""))
	...
	if(!isNaN(obj.value))
{
  ...
  if( sell_Buy_Flag == "BR" || sell_Buy_Flag == "SBR")//Modified by Anil.k for Spot Rates
	{
	  marginvalue1 = round(obj.value);
			if(marginvalue1 != ''  ){
               obj.value = round(marginvalue1);
	...
	{
	   discountvalue1 = round(obj.value);
			if(discountvalue1 != ''  ){
               obj.value = round(discountvalue1);
         discountTypes =  document.getElementById("discountTypes"+info+"4"+breaks).value;
		 if(discountTypes == "A")
		{
            discountvalue  = round(  parseFloat(rsrValue) - parseFloat(calSellRate) ) ;
		}
		else if(discountTypes == "P")
		{
      discountvalue =  round(((parseFloat(rsrValue) - parseFloat(calSellRate))*100)/parseFloat(rsrValue)) ;
		}
		document.getElementById("DiscountVal"+info+"5"+breaks).value = round(parseFloat(discountvalue)) ;
	}
}
}else{
alert("Please Enter Numeric Values Only.");
...
function selectDestAll(laneno,obj)
{	
  	var destChk	  = document.getElementsByName("destChkBox"+laneno);
	//alert(destChk.length);
	if(obj.checked){
	for(var i=0;i<destChk.length;i++)
		{
			destChk[i].checked=true;
...
function setAllValuesOnLoad()
{
   ...
   for(var fr=0;fr<frtsize;fr++){
	...
	if(isMarginTestPerformed!='Y')
	{
		...
		{
			if(submitObj[i].value=='Next>>'||submitObj[i].value=='Update') // Added by Gowtham
...
function calcSellRate(rate,index,type,chargeType,rowNo,legNo,laneno,obj)
{
...
function calcSellRate1(rate,index,type,chargeType,rowNo,legNo,sellBuyFlag,laneno,obj)
{
...
//@@Added by Kameswari for the WPBN issue-146448
			...
			//@@WPBN issue-146448-->
//@ added by silpa for chek all freightbreaks
function selectAllFreightBreaks(obj){


var breaks = document.getElementsByName("breakChk");
...
for(var i=0;i<length;i++){
     selectedbreaks = selectedbreaks +i+",";
      document.getElementsByName("breakChk")[i].checked=true;
}
}
else{
	...
	for(var i=0;i<length;i++){
     selectedbreaks = "";
      document.getElementsByName("breakChk")[i].checked=false;
}
}
 document.forms[0].selectedIndexList.value = selectedbreaks;
...
<%}else{%>
<BODY onLoad="initialize();setAllValues();setAllValuesOnLoad();roundAll();setSelectedIndexValueOnLoad();">
<%}%>
...
<%			StringBuffer   attentionTo            =   null;
			...
			<tr class="formdata">
              <td colspan="<%=serviceColspan%>" align='right'>SALES PERSON: </td>
...
<%}
				...
				//	System.out.println("zone  rocky.......   "+legRateDetails.getZone());
					%>
					<tr class="formheader" cellpadding="4" >	   <!-- // Modified by Rakesh on 24-03-2011 -->
				  
				  <td align='center'><input type="checkbox" name='select<%=fr%>' onclick='selectAll(<%=fr%>)'></td>
					<!--   <td colspan="<%=colspan-1%>" align='center'><%=legRateDetails.getOrigin()%>-ORIGIN CHARGES</td> -->
					<td colspan="<%=colspan-1%>" align='center'><%=legRateDetails.getOrgFullName()%>-ORIGIN CHARGES</td>
					   <td colspan ="<%=tempCount%>"></td>
					</tr>
					<%}checkedFlag				=	false;
...
%>	<!-- @@ Commented by subrahmanyam for 180161 -->
				...
				<!-- @@ Added by subrahmanyam for 180161 -->
					  ...
					  <td>
					  <select class="select"  name="originMarginType<%=fr%><%=j%>" size="1" onChange="enableDisableMarginType(this,'originDiscount',<%=j%>,'Discount','originMargin',<%=originList%>,'Margin','origin',<%=k%>,0,'<%=chargesDOB.getSellBuyFlag()%>',<%=fr%>);return calcSellRate(<%=chargeInfo.getBuyRate()%>,<%=j%>,'Margin','origin',<%=k%>,0,<%=fr%>,this)" >
				...
				<%}else{
					...
					<td align='center'><input class="text" type="text" value="<%=chargeInfo.getMargin()<0?0:chargeInfo.getMargin()%>" size=5 name='originMargin<%=fr%><%=j%>'  <%=originMarginDisabled%> onblur='calcSellRate1(<%=chargeInfo.getBuyRate()%>,<%=j%>,"Margin","origin",<%=k%>,0,"<%=chargesDOB.getSellBuyFlag()%>","<%=fr%>",this)' onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','originDiscount<%=fr%>',<%=j%>,'Discount')" maxlength='10'>
					  </td>
				<%}%>
					  <%}
%>
					...
					<%if("B".equalsIgnoreCase(chargesDOB.getSellBuyFlag())||"BC".equalsIgnoreCase(chargesDOB.getSellBuyFlag()))
					 ...
					 <td>
					  ...
					  <%}
						...
						{%>
						<td>
					  <select class="select"  name="originDiscountType<%=fr%><%=j%>"  size="1" id='noRound' onChange="enableDisableMarginType(this,'originMargin',<%=j%>,'Margin','originDiscount',<%=originList1%>,'Discount','origin',<%=k%>,0,'<%=chargesDOB.getSellBuyFlag()%>',<%=fr%>);calcSellRate(<%=chargeInfo.getRecOrConSellRrate()%>,<%=j%>,'Discount','origin',<%=k%>,0,<%=fr%>,this)">
						  <option value="A" <%="A".equals(chargeInfo.getDiscountType())?"selected":""%>>Absolute</option>
						  <option value="P" <%="P".equals(chargeInfo.getDiscountType())?"selected":""%>>Percent</option>
						</select>
					  </td>
					  <%}%>
					  <td><input class="text" type="text" value="<%=df1.format(new Double(chargeInfo.getDiscount()))%>" size=5 name='originDiscount<%=fr%><%=j%>' onblur="calcSellRate(<%=chargeInfo.getRecOrConSellRrate()%>,<%=j%>,'Discount','origin',<%=k%>,0,
					 <%=fr%>,this)" <%=originDiscDisabled%> onchange="enableDisableMargin(this,'<%=chargeInfo.getDiscount()%>','originMargin<%=fr%>',<%=j%>,'Margin')" maxlength='10'> </td>
...
<%
			...
			%>
			<tr class="formheader"  width="100%"cellpadding="4" cellspacing="1" align='center'  >
			  <td >Origin </td>
			  <td >Destination</td>
			  <td>Incoterms</td>
			  <td >Carrier<br><br><input type="checkbox" name="carrier" onClick="selectCSF(this)" value="C"></td><!-- Added by Anil.k to display in 4th screen or not --><input type="hidden" name="selectCarrier" value="unchecked">
			  <td>Service Level<br><input type="checkbox" name="serviceLevel" onClick="selectCSF(this)" value="S"></td><!-- Added by Anil.k to display in 4th screen or not --><input type="hidden" name="selectService" value="unchecked">
			  <td>Frequency<br><br><input type="checkbox" name="frequency" onClick="selectCSF(this)" value="F"></td><!-- Added by Anil.k to display in 4th screen or not --><input type="hidden" name="selectFrequency" value="unchecked">
			  <td>Transit Time&nbsp<br><input type="checkbox" name="apprTranistTime" onClick="selectCSF(this)" value="T"></td>
			  <input type="hidden" name="selectTransitTime" value="unchecked">
			  <!--Modified by Rakesh on 24-03-2011-->
			  <td>Freight Validity<br><input type="checkbox" name="freightValidity" onClick="selectCSF(this)" value="V"></td>
...
<%chargeInfoList = ((MultiQuoteFreightLegSellRates)charges.get(0)).getFreightChargesList();
					int infoSize	=	chargeInfoList.size();
						int brkCount	=	0;
							chargeInfo	=	(MultiQuoteChargeInfo)chargeInfoList.get(0);
							String checked_Flag = "";
							String[] checked_Flag_Split = null;%>
							
						
							
							 <td>BR/RSR<br><br>
					<INPUT TYPE="checkbox" NAME="brrsr" onclick="selectAllFreightBreaks(this)">
					</td>
							<%System.out.println("checked_flag--------"+chargeInfo.getChecked_Flag());
							
							for(String s : chargeInfo.getMultiBreakPoints()){
								checked_Flag_Split = chargeInfo.getChecked_Flag().split(",");
						...
						System.out.println("selectedIndexList-----"+selectedIndexList.contains(",null"));
       if(chargeInfo.getChecked_Flag()!= null && !"-".equals(checked_Flag_Split[brkCount]) && !"".equals(checked_Flag_Split[brkCount]) && "Y".equals(checked_Flag_Split[brkCount]) && (selectedIndexList.contains(",null"))){
		   System.out.println("in checked");
				...
				//	System.out.println("1967----"+checked_Flag);%>
        ...
        <%=	(!"A FREIGHT RATE".equalsIgnoreCase(chargeInfo.getMultiRateDescriptions()[brkCount])) ?s.substring(0,s.length()-2):s%>
		 ...
		 <%=	(!"A FREIGHT RATE".equalsIgnoreCase(chargeInfo.getMultiRateDescriptions()[brkCount]) && s.length()>5)?s.substring(0,3)+s.substring(5):s%>
         <%}%>
			<br><%//System.out.println("selectedIndexList-------"+selectedIndexList+"----------"+brkCount+"*********");%>
						
                              <%=chargeInfo.getBasis().split(",")[brkCount]%><br>
	                         <%count++;%>
							<input type='checkbox'  name='breakChk' onclick='setSelectedIndexValue(this,<%=brkCount%>)'
							<%=(selectedIndexList!= null && !selectedIndexList.contains(",null"))?(selectedIndexList.contains(","+Integer.toString(brkCount)+",")?"checked":""):checked_Flag%> >
...
%>						   
	    <input type='hidden' name='marginTestFailed' value='<%=( chargeInfo.isMarginTestFailed())?"Y":"N"%>'/>
						   ...
						   <td rowspan="<%=rowSpan%>"><font color='<%=freightcolor%>'><%=chargeInfo.getFrequency()!=null || "null".equalsIgnoreCase(chargeInfo.getFrequency())?chargeInfo.getFrequency():""%></font></td>
						   ...
						   <%for (int bre=0;bre<7;bre++){
							...
							<%}else if(bre==2 && !"RSR".equals(chargeInfo.getSellBuyFlag())){
                            ...
                            <%
								...
								String  disabled="";
								if((bre==1 && !"RSR".equals(chargeInfo.getSellBuyFlag())) || (bre==4 && !"SBR".equals(chargeInfo.getSellBuyFlag()) && !"BR".equals(chargeInfo.getSellBuyFlag()))){
						...
						onChange='setEnableDisableAbsPers(<%=info%>,<%=bre%>,<%=breaksLen%>,"<%=chargeInfo.getSellBuyFlag()%>");calSellRateOnAbsPerChange(this,<%=bre%>,<%=info%>,<%=breaks%>,<%=buyRate.equals("-")?0:buyRate%>,<%=sellRate.equals("-")?0:sellRate%>)'  <%=disabled%> >
						  ...
						  <option value="P" <%=tempData!=null?"P".equals(tempData[breaks])?"selected":"":"selected"%>>Percent</option>
									...
									<%}else if((bre==2 && !"RSR".equals(chargeInfo.getSellBuyFlag()))|| (bre==5 && !"SBR".equals(chargeInfo.getSellBuyFlag()) && !"BR".equals(chargeInfo.getSellBuyFlag()))){
									...
									id='<%=bre==2?"MarginVal":"DiscountVal"%><%=info%><%=bre%><%=breaks%>'
							value='<%=tempData!=null?tempData[breaks]:"0.0"%>' <%=tempData!=null&&"-".equals(tempData[breaks])?readOnly:""%> onBlur='calaculateSellRate(this,<%=info%>,<%=bre%>,<%=breaks%>,<%=buyRate.equals("-")?0:buyRate%>,<%=sellRate.equals("-")?0:sellRate%>)' <%=marginDiscountdisabled%> /></td>
									...
									<td><input class='text' size='2' type='text' name='calsellrate<%=info%>'  value='-' readonly ></td>
											<%}else{%>
									<td><input class='text' size='5' type='text' name='calsellrate<%=info%>' id='calsellrate<%=info%><%=bre%><%=breaks%>'  value='<%=tempData!=null && tempData[breaks] != null?tempData[breaks]:"0.0"%>' onBlur ='reverseCallSellRate(this,<%=info%>,<%=breaks%>,<%=buyRate.equals("-")?0:buyRate%>,
									<%=sellRate.equals("-")?0:sellRate%>)'></td>
										<%}}else{
											   if(bre== 0){%>
							...
							sellRate	= "";
							 ...
							 <!--Modified by Rakesh on 25-03-2010-->
					<td rowspan="<%=rowSpan-1%>"><font color='<%=freightcolor%>'><%=chargeInfo.getRatio()!=null?chargeInfo.getRatio():""%></font></td>
							<%}}%>
...
<%
				...
				for(int j=0;j<destChargesSize;j++)
				{
					...
					<tr class="formheader" cellpadding="4" >
					 <td align='center'><input type="checkbox" name='destSelect<%=fr%>' onclick='selectDestAll(<%=fr%>,this)'></td> 
					  <td colspan="<%=colspan-1%>" align='center'><%=legRateDetails.getDestFullName()%>-DESTINATION CHARGES</td>
					<td colspan ="<%=tempCount%>"></td>
					</tr>
...
%>			  		<!-- @@ Commented by subrahmanyam for 180161 -->
				...
				<!-- @@Ended by subrahmanyam for 180161 -->
					<td>
					  <select class="select"  name="destMarginType<%=fr%><%=j%>" size="1"  onChange="enableDisableMarginType(this,'destDiscount',<%=j%>,'Discount','destMargin',<%=destList%>,'Margin','dest',<%=k%>,0,'<%=chargesDOB.getSellBuyFlag()%>',<%=fr%>);calcSellRate(<%=chargeInfo.getBuyRate()%>,<%=j%>,'Margin','dest',<%=k%>,0,<%=fr%>,this)" >
						  <option value="A" <%="A".equals(chargeInfo.getMarginType())?"selected":""%>>Absolute</option>
						  <option value="P" <%="P".equals(chargeInfo.getMarginType())?"selected":""%>>Percent</option>
					  </select>
					  </td>
					...
					//	int mTCount4=MarginTestCount1.intValue();
								...
								<td align='center'><font color="<%=color%>"><input class="text" type="text" value="<%=chargeInfo.getMargin()%>" size=5 name='destMargin<%=fr%><%=j%>' onblur='calcSellRate1(<%=chargeInfo.getBuyRate()%>,<%=j%>,"Margin","dest",<%=k%>,0,"<%=chargesDOB.getSellBuyFlag()%>","<%=fr%>",this)' onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','destDiscount<%=fr%>',<%=j%>,'Discount')" <%=destMarginDisabled%> maxlength='10'></font></td>

					  <%}else{
								session.setAttribute("MarginTestCount",MarginTestCount);%>
								<td align='center'><font color="<%=color%>"><input class="text" type="text" value="<%=chargeInfo.getMargin()<0?0:chargeInfo.getMargin()%>" size=5 name='destMargin<%=fr%><%=j%>' onblur='calcSellRate1(<%=chargeInfo.getBuyRate()%>,<%=j%>,"Margin","dest",<%=k%>,0,"<%=chargesDOB.getSellBuyFlag()%>","<%=fr%>",this)' onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','destDiscount<%=fr%>',<%=j%>,'Discount')" <%=destMarginDisabled%> maxlength='10'></font></td>
					  <%}%>
...
%>						<td><font color="<%=color%>"><%=df.format(new Double(chargeInfo.getRecOrConSellRrate()))%></font></td>
					  ...
					  <%}
						...
						{	%>
                          <td>
						<select class="select"  name="destDiscountType<%=fr%><%=j%>" size="1" onChange="enableDisableMarginType(this,'destMargin',<%=j%>,'Margin','destDiscount',<%=destList1%>,'Discount','dest',<%=k%>,0,'<%=chargesDOB.getSellBuyFlag()%>',<%=fr%>);calcSellRate(<%=chargeInfo.getRecOrConSellRrate()%>,<%=j%>,'Discount','dest',<%=k%>,0,<%=fr%>,this)" >
						  <option value="A" <%="A".equals(chargeInfo.getDiscountType())?"selected":""%>>Absolute</option>
						  <option value="P" <%="P".equals(chargeInfo.getDiscountType())?"selected":""%>>Percent</option>
						</select>
					  </td>
					  ...
					  onblur="calcSellRate(<%=chargeInfo.getRecOrConSellRrate()%>,<%=j%>,'Discount','dest',<%=k%>,0,
					  <%=fr%>,this)" <%=destDiscDisabled%> onchange="enableDisableMargin(this,'<%=chargeInfo.getDiscount()%>','destMargin<%=fr%>',<%=j%>,'Margin')" maxlength='10'></font></td>
...
<%
	 ...
	 if("Modify".equalsIgnoreCase(operation)) {%>
			<td	colspan="2"><table cellpadding="4" cellspacing="1" width="100%"><tr class='formlabel'><td align='left'><%=shipmentMode%> QUOTE WITH CHARGE (<%=quoteName%>,<%=quoteStatus%>,<%=completeFlag%>)- <%=operation!=null?operation.toUpperCase():""%></td><td align=right><!-- Screen Id --></td>			
			</tr></table></td>
			<%}else { %>
			<td	colspan="2"><table cellpadding="4" cellspacing="1" width="100%"><tr class='formlabel'><td align='left'><%=shipmentMode%>  QUOTE WITH CHARGE - <%=operation!=null?operation.toUpperCase():""%></td><td align=right><!-- Screen Id --></td>
			</tr></table></td>
             <!-- @@WPBN issue-167677 -->
			<%}%>
		  </tr></table>
...
<%			StringBuffer   attentionTo            =   null;
			...
			<tr class="formdata">
			 ...
			 <tr class="formdata">
			  ...
			  <td colspan="<%=infoColspan%>" ><%=masterDOB.getIncoTermsId()[0]!=null?masterDOB.getIncoTermsId()[0]:""%></td>
...
%>	<!-- @@ Commented by subrahmanyam for 180161 -->
				...
				<%if("Yes".equalsIgnoreCase(MarginTest) || "<<Back".equalsIgnoreCase(submit)||"Y".equalsIgnoreCase(chargesDOB.getSelectedFlag())){
					%>
						<td align='center'><input class="text" type="text" value="<%=chargeInfo.getMargin()%>" size=5 name='originMargin<%=fr%><%=j%>'  <%=originMarginDisabled%> onblur='calcSellRate1(<%=chargeInfo.getBuyRate()%>,<%=j%>,"Margin","origin",<%=k%>,0,"<%=chargesDOB.getSellBuyFlag()%>","<%=fr%>",this)' onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','originDiscount<%=fr%>',<%=j%>,'Discount')" maxlength='10'>
...
%>
					...
					<%if("B".equalsIgnoreCase(chargesDOB.getSellBuyFlag())||"BC".equalsIgnoreCase(chargesDOB.getSellBuyFlag()))
					 ...
					 <td>
					  ...
					  <%}%>
					  <td><input class="text" type="text" value="<%=df1.format(new Double(chargeInfo.getDiscount()))%>" size=5 name='originDiscount<%=fr%><%=j%>' onblur="calcSellRate(<%=chargeInfo.getRecOrConSellRrate()%>,<%=j%>,'Discount','origin',<%=k%>,0,<%=fr%>,this)" <%=originDiscDisabled%> onchange="enableDisableMargin(this,'<%=chargeInfo.getDiscount()%>','originMargin<%=fr%>',<%=j%>,'Margin')" maxlength='10'></td>
...
<%
				...
				{ if(j==0 && destChargesSize >0){%>
					<tr class="formheader" cellpadding="4" >
					<td align='center'><input type="checkbox" name='select<%=fr%>' onclick='selectDestAll(<%=fr%>,this)'></td>
					  <td colspan="<%=colspan-1%>" align='center'><%=legRateDetails.getDestFullName()%>-DESTINATION CHARGES</td>
				  ...
				  <td width="18%" align="right" colspan="<%=colspan%>">
				   ...
				   <% if("Y".equals(request.getParameter("isEscalated"))) {%>
					...
					<input class="input" name="submit"  type="submit" value="Update" onclick='setSubmitValue("Next>>")'>  <!--Added by Gowtham  -->
                                    <%}else if(!"Modify".equalsIgnoreCase(updateValue)){%>
...
<%}
			if(fromWhere!=null)
			{if(!"Modify".equalsIgnoreCase(updateValue)){
%>
...
<%}%>				 
				  ...
				  <input type="hidden" name="subOperation" value="SELLCHARGES" >
				   ...
				   <input type="hidden" name="updateValue" value=<%=updateValue%> >
