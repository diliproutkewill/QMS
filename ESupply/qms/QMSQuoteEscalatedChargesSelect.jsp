...
esupply - v 1.x.
Program	Name		: QMSQuoteEscalatedChargesSelect.jsp
Product Name		: QMS
Module Name			: Quote
Task				: Adding/Modify
Date started		:
...
<%!
  ...
  private static final String FILE_NAME	=	"QMSQuoteEscalatedChargesSelect.jsp";
...
function setAllValuesOnLoad()
{
	...
	if(isMarginTestPerformed !='Y')
	{
		...
		{
			if(submitObj[i].value=='Update') // Added by Gowtham for QuoteEscalted Modify issue.
...
function calcSellRate(rate,index,type,chargeType,rowNo,legNo)
{
	var noOfLegs	=	<%=chargesSize%>;
...
function calcSellRate1(rate,index,type,chargeType,rowNo,legNo,sellBuyFlag)
{
	var noOfLegs	=	<%=chargesSize%>;
	bcFlag = sellBuyFlag;
	var object=  document.getElementsByName(chargeType+type+index);
...
document.forms[0].action='QMSReportController?Operation=Escalated&fromSummary=Yes&nextOperation=Escalatedpagging&subOperation=html/excel&radiobutton=self';
...
function setSubmitValue(val)
{
	...
	document.forms[0].radiobutton.value = "self";
...
<form method='post' action='QMSQuoteController' onsubmit='disableUnChecked();  return checkMarginTest();'> <!-- Commented By Gowtham -->
	...
	<td>
		<table width="100%" cellpadding="0" cellspacing="0">
		  ...
		  <%if("Modify".equalsIgnoreCase(operation)) {%>
			...
			<%}else { %>
			<td	colspan="2"><table cellpadding="4" cellspacing="1" width="100%"><tr class='formlabel'><td align='left'><%=shipmentMode%> FREIGHT QUOTE - Escalated View</td><td align=right><!-- Screen Id --></td></tr></table></td>
             <!-- @@WPBN issue-167677 -->
			<%}%>
		  </tr></table>
<%
...
%>	<!-- @@ Commented by subrahmanyam for 180161 -->
				...
				<!-- @@ Added by subrahmanyam for 180161 -->
					  ...
					  <td>
					  <select class="select"  name="originMarginType<%=j%>" size="1" onChange="enableDisableMarginType(this,'originDiscount',<%=j%>,'Discount','originMargin',<%=originList%>,'Margin','origin',<%=k%>,0,'<%=chargesDOB.getSellBuyFlag()%>');return calcSellRate(<%=chargeInfo.getBuyRate()%>,<%=j%>,'Margin','origin',<%=k%>,0)" >
		...
		<!-- @@ Commented by subrahmanyam for the Enhancement 154381 on 03/02/09 -->
					<!-- <td align='center'><input class="text" type="text" value="<%=df1.format(new Double(chargeInfo.getMargin()))%>" size=5 name='originMargin<%=j%>'  <%=originMarginDisabled%> onblur='calcSellRate(<%=chargeInfo.getBuyRate()%>,<%=j%>,"Margin","origin",<%=k%>,0)' onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','originDiscount',<%=j%>,'Discount')" maxlength='10'>
					  </td>
<!-- @@ Added by subrahmanyam for the Enhancement 154381 on 03/02/09 -->
			...
			//	int mTCount1=MarginTestCount1.intValue();
					%>
						<td align='center'><input class="text" type="text" value="<%=chargeInfo.getMargin()%>" size=5 name='originMargin<%=j%>'  <%=originMarginDisabled%> onblur='calcSellRate1(<%=chargeInfo.getBuyRate()%>,<%=j%>,"Margin","origin",<%=k%>,0,"<%=chargesDOB.getSellBuyFlag()%>")' onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','originDiscount',<%=j%>,'Discount')" maxlength='10'>

				<%}else{
					session.setAttribute("MarginTestCount",MarginTestCount);%>
					<td align='center'><input class="text" type="text" value="<%=chargeInfo.getMargin()<0?0:chargeInfo.getMargin()%>" size=5 name='originMargin<%=j%>'  <%=originMarginDisabled%> onblur='calcSellRate1(<%=chargeInfo.getBuyRate()%>,<%=j%>,"Margin","origin",<%=k%>,0,"<%=chargesDOB.getSellBuyFlag()%>")' onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','originDiscount',<%=j%>,'Discount')" maxlength='10'>
					  </td>
				<%}%>
					...
					<%if("B".equalsIgnoreCase(chargesDOB.getSellBuyFlag())||"BC".equalsIgnoreCase(chargesDOB.getSellBuyFlag()))
					 ...
					 <td>
					  ...
					  <%}
						...
						{%>
						<td>
					  <select class="select"  name="originDiscountType<%=j%>"  size="1" id='noRound' onChange="enableDisableMarginType(this,'originMargin',<%=j%>,'Margin','originDiscount',<%=originList1%>,'Discount','origin',<%=k%>,0,'<%=chargesDOB.getSellBuyFlag()%>');calcSellRate(<%=chargeInfo.getRecOrConSellRrate()%>,<%=j%>,'Discount','origin',<%=k%>,0)">
						  <option value="A" <%="A".equals(chargeInfo.getDiscountType())?"selected":""%>>Absolute</option>
						  <option value="P" <%="P".equals(chargeInfo.getDiscountType())?"selected":""%>>Percent</option>
						</select>
					  </td>
					  <%}%>
					  <td><input class="text" type="text" value="<%=df1.format(new Double(chargeInfo.getDiscount()))%>" size=5 name='originDiscount<%=j%>' onblur="calcSellRate(<%=chargeInfo.getRecOrConSellRrate()%>,<%=j%>,'Discount','origin',<%=k%>,0)" <%=originDiscDisabled%> onchange="enableDisableMargin(this,'<%=chargeInfo.getDiscount()%>','originMargin',<%=j%>,'Margin')" maxlength='10'></td>
					  ...
					  <!-- @@ Added by subrahmanyam for the Enhancement 180161 on 31/08/09 -->
					  <%if("B".equalsIgnoreCase(chargesDOB.getSellBuyFlag()) || "S".equalsIgnoreCase(chargesDOB.getSellBuyFlag())){%>
					  <td><input class="text" type="text" id='chargeSellrate' value="<%=round(chargeInfo.getSellRate())%>" size=5 readOnly name='originSellRate<%=j%>'></td>
					  <%}else{%>
					  <td><input class="text" type="text" id='chargeSellrate' value="<%=df1.format(new Double(chargeInfo.getSellRate()))%>" size=5 readOnly name='originSellRate<%=j%>'></td>
					  <%}%>
...
<%
					...
					<td><font color="<%=color%>"><%=(chargeInfo.getRateDescription()!=null)?chargeInfo.getRateDescription().substring(0,chargeInfo.getRateDescription().length()-3):"FREIGHT RATE"%></font></td>
					 <%}
						}%>
					...
					<td>MIN</td>
					 ...
					 <%}else{%>
					  <%=chargeInfo.getBreakPoint().length()>5?toTitleCase(chargeInfo.getBreakPoint().substring(0,4)):toTitleCase(chargeInfo.getBreakPoint())%></font></td>
					<%}}
						%>
					  <td><font color="<%=color%>"><%=chargeInfo.getCurrency()!=null?chargeInfo.getCurrency():""%></font></td>
<%					if("Y".equals(buyRatePermission))
...
%>			  		  <td><font color="<%=color%>"><%=df.format(new Double(chargeInfo.getBuyRate()))%></font></td>
					  ...
					  <select class="select"  name="frtMargin<%=i%>Type<%=j%>" size="1" onChange="enableDisableMarginType(this,'frtDiscount<%=i%>',<%=j%>,'Discount','frtMargin<%=i%>',<%=rateList%>,'Margin','frt',<%=k%>,<%=i%>,'<%=chargesDOB.getSellBuyFlag()%>');calcSellRate(<%=chargeInfo.getBuyRate()%>,<%=j%>,'Margin','frt',<%=k%>,<%=i%>)" >
						  <option value="A" <%="A".equals(chargeInfo.getMarginType())?"selected":""%>>Absolute</option>
						  <option value="P" <%="P".equals(chargeInfo.getMarginType())?"selected":""%>>Percent</option>
					  </select></td>
					<%
					...
					<%if("Yes".equalsIgnoreCase(MarginTest) || "<<Back".equalsIgnoreCase(submit)||"Y".equalsIgnoreCase(chargesDOB.getSelectedFlag())){%>
							...
							<td align=center><font color="<%=color%>"><input class="text" type="text" value="<%=chargeInfo.getMargin()%>" size=5 name='frtMargin<%=i%><%=j%>'  onblur="calcSellRate(<%=chargeInfo.getBuyRate()%>,<%=j%>,'Margin','frt',<%=k%>,<%=i%>)" <%=frtMarginDisabled%> onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','frtDiscount<%=i%>',<%=j%>,'Discount')" maxlength='10'></font></td>

					<%}else{%>
							<td align=center><font color="<%=color%>"><input class="text" type="text" value="<%=chargeInfo.getMargin()<0?0:chargeInfo.getMargin()%>" size=5 name='frtMargin<%=i%><%=j%>'  onblur="calcSellRate(<%=chargeInfo.getBuyRate()%>,<%=j%>,'Margin','frt',<%=k%>,<%=i%>)" <%=frtMarginDisabled%> onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','frtDiscount<%=i%>',<%=j%>,'Discount')" maxlength='10'></font></td>
					<%}%>
<!-- @@ Ended by subrahmanyam for the Enhancement 154381 on 03/02/09 -->
					<%}
					  else
            ...
            if(count>0||"Y".equalsIgnoreCase(chargesDOB.getSelectedFlag()))
							{%>
                         <!-- @@ Commented by subrahmanyam for the Enhancement 154381 on 03/02/09 -->
                         <!-- <td align=center><font color="<%=color%>"><input class="text" type="text" value="<%=df1.format(new Double(chargeInfo.getMargin()))%>" size=5 name='frtMargin<%=i%><%=j%>'  onblur="calcSellRate(<%=chargeInfo.getBuyRate()%>,<%=j%>,'Margin','frt',<%=k%>,<%=i%>)" <%=frtMarginDisabled%> onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','frtDiscount<%=i%>',<%=j%>,'Discount')"  maxlength='10'></font></td> -->
<!-- @@ Added by subrahmanyam for the Enhancement 154381 on 03/02/09 -->
...
<%				         }
					      ...
					      {
							  %>
                            <td align=center><font color="<%=color%>"><input class="text" type="text" value="0.0" size=5 name='frtMargin<%=i%><%=j%>'  onblur="calcSellRate(<%=chargeInfo.getBuyRate()%>,<%=j%>,'Margin','frt',<%=k%>,<%=i%>)" <%=frtMarginDisabled%> onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','frtDiscount<%=i%>',<%=j%>,'Discount')"  maxlength='10'></font></td>
...
%>
					...
					<%}%>
					 <%
						  if("FREIGHT RATE".equalsIgnoreCase(chargeInfo.getRateDescription()))
						{%>
            <td><font color="<%=color%>"><input class="text" type="text" value="<%=df1.format(new Double(chargeInfo.getDiscount()))%>" size='5' name='frtDiscount<%=i%><%=j%>' onblur="calcSellRate(<%=chargeInfo.getRecOrConSellRrate()%>,<%=j%>,'Discount','frt',<%=k%>,<%=i%>)" <%=frtDiscDisabled%> onchange="enableDisableMargin(this,'<%=chargeInfo.getDiscount()%>','frtMargin<%=i%>',<%=j%>,'Margin')" maxlength='10'></font></td>
					  <%}
					  else
						{
            //if(count>0)//@@Commented and Modified by Kameswari for the WPBN issue-143250
           ...
           <%				         }
					      ...
					      {
							  %>
                           <td><font color="<%=color%>"><input class="text" type="text" value="0.0" size='5' name='frtDiscount<%=i%><%=j%>' onblur="calcSellRate(<%=chargeInfo.getRecOrConSellRrate()%>,<%=j%>,'Discount','frt',<%=k%>,<%=i%>)" <%=frtDiscDisabled%> onchange="enableDisableMargin(this,'<%=chargeInfo.getDiscount()%>','frtMargin<%=i%>',<%=j%>,'Margin')" maxlength='10'></font></td>
...
<!-- <td align='center'><font color="<%=color%>"><input class="text" type="text" value="<%=df1.format(new Double(chargeInfo.getSellRate()))%>" size=5 readOnly name='frtSellRate<%=i%><%=j%>'></font></td> -->
           <%if("B".equalsIgnoreCase(chargesDOB.getSellBuyFlag()) || "S".equalsIgnoreCase(chargesDOB.getSellBuyFlag())){%>
			<td align='center'><font color="<%=color%>"><input class="text" type="text" value="<%=round(chargeInfo.getSellRate())%>" size=5 readOnly name='frtSellRate<%=i%><%=j%>'></font></td>
			<%}else{%>
			<td align='center'><font color="<%=color%>"><input class="text" type="text" value="<%=df1.format(new Double(chargeInfo.getSellRate()))%>" size=5 readOnly name='frtSellRate<%=i%><%=j%>'></font></td>
			<%}%>
...
%>			  		<!-- @@ Commented by subrahmanyam for 180161 -->
				...
				<!-- @@Ended by subrahmanyam for 180161 -->
					<td>
					  <select class="select"  name="destMarginType<%=j%>" size="1"  onChange="enableDisableMarginType(this,'destDiscount',<%=j%>,'Discount','destMargin',<%=destList%>,'Margin','dest',<%=k%>,0,'<%=chargesDOB.getSellBuyFlag()%>');calcSellRate(<%=chargeInfo.getBuyRate()%>,<%=j%>,'Margin','dest',<%=k%>,0)" >
						  <option value="A" <%="A".equals(chargeInfo.getMarginType())?"selected":""%>>Absolute</option>
						  <option value="P" <%="P".equals(chargeInfo.getMarginType())?"selected":""%>>Percent</option>
					  </select>
					  </td>
					  <!-- @@ Commented by subrahmanyam for the Enhancement 154381 on 03/02/09 -->
					  <!-- <td align='center'><font color="<%=color%>"><input class="text" type="text" value="<%=df1.format(new Double(chargeInfo.getMargin()))%>" size=5 name='destMargin<%=j%>' onblur='calcSellRate(<%=chargeInfo.getBuyRate()%>,<%=j%>,"Margin","dest",<%=k%>,0)' onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','destDiscount',<%=j%>,'Discount')" <%=destMarginDisabled%> maxlength='10'></font></td> -->
<!-- @@ Added by subrahmanyam for the Enhancement 154381 on 03/02/09 -->
						<%if("Yes".equalsIgnoreCase(MarginTest) || "<<Back".equalsIgnoreCase(submit)||"Y".equalsIgnoreCase(chargesDOB.getSelectedFlag()))
						{
					...
					//	int mTCount4=MarginTestCount1.intValue();
								...
								<td align='center'><font color="<%=color%>"><input class="text" type="text" value="<%=chargeInfo.getMargin()%>" size=5 name='destMargin<%=j%>' onblur='calcSellRate1(<%=chargeInfo.getBuyRate()%>,<%=j%>,"Margin","dest",<%=k%>,0,"<%=chargesDOB.getSellBuyFlag()%>")' onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','destDiscount',<%=j%>,'Discount')" <%=destMarginDisabled%> maxlength='10'></font></td>

					  <%}else{
								session.setAttribute("MarginTestCount",MarginTestCount);%>
								<td align='center'><font color="<%=color%>"><input class="text" type="text" value="<%=chargeInfo.getMargin()<0?0:chargeInfo.getMargin()%>" size=5 name='destMargin<%=j%>' onblur='calcSellRate1(<%=chargeInfo.getBuyRate()%>,<%=j%>,"Margin","dest",<%=k%>,0,"<%=chargesDOB.getSellBuyFlag()%>")' onchange="enableDisableMargin(this,'<%=chargeInfo.getMargin()%>','destDiscount',<%=j%>,'Discount')" <%=destMarginDisabled%> maxlength='10'></font></td>
					  <%}%>
<!-- @@ Ended by subrahmanyam for the Enhancement 154381 on 03/02/09 -->
<%					}
%>						<td><font color="<%=color%>"><%=df.format(new Double(chargeInfo.getRecOrConSellRrate()))%></font></td>
					  ...
					  <%}
						...
						{	%>
                          <td>
						<select class="select"  name="destDiscountType<%=j%>" size="1" onChange="enableDisableMarginType(this,'destMargin',<%=j%>,'Margin','destDiscount',<%=destList1%>,'Discount','dest',<%=k%>,0,'<%=chargesDOB.getSellBuyFlag()%>');calcSellRate(<%=chargeInfo.getRecOrConSellRrate()%>,<%=j%>,'Discount','dest',<%=k%>,0)" >
						  <option value="A" <%="A".equals(chargeInfo.getDiscountType())?"selected":""%>>Absolute</option>
						  <option value="P" <%="P".equals(chargeInfo.getDiscountType())?"selected":""%>>Percent</option>
						</select>
					  </td>
					  ...
					  onblur="calcSellRate(<%=chargeInfo.getRecOrConSellRrate()%>,<%=j%>,'Discount','dest',<%=k%>,0)" <%=destDiscDisabled%> onchange="enableDisableMargin(this,'<%=chargeInfo.getDiscount()%>','destMargin',<%=j%>,'Margin')" maxlength='10'></font></td>
					  ...
					  <!-- Added for 180161 -->
					  <%if("B".equalsIgnoreCase(chargesDOB.getSellBuyFlag()) || "S".equalsIgnoreCase(chargesDOB.getSellBuyFlag())){%>
					  <td><font color="<%=color%>"><input class="text" type="text" id='Destrates' value="<%=round(chargeInfo.getSellRate())%>" size=5 readOnly name='destSellRate<%=j%>'></font></td>
					  <%}else{%>
					  <td><font color="<%=color%>"><input class="text" type="text" value="<%=df1.format(new Double(chargeInfo.getSellRate()))%>" size=5 readOnly name='destSellRate<%=j%>'></font></td>
					  <%}%>
					  <!-- Ended for 180161 -->
		...
		<tr class='text'>
				...
				<input class="input" name="submit" type="submit" value="Close" onclick='closeandContinue()'>
				  ...
				  <input type="hidden" name="quoteName" value="<%= quoteName!=null? quoteName:""%>">
				   ...
				   <input type="hidden" name ="radiobutton" value="">
