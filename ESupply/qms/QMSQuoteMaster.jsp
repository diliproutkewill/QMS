...
<script language="javascript" src="<%=request.getContextPath()%>/html/dynamicContent.js"></script>
	<script language="javascript" src="<%=request.getContextPath()%>/html/autoCompleteAJAX.js"></script>
...
//commented by subrahmanyam for issue id: 185101 on 01-oct-09
	...
	{
		alert('Please Select the Customer Attention To');
		return false;
	}
...
function setSubmit(obj)
	{
document.forms[0].submitt1.value=obj.value;
       if(validate())
		{
		   document.QuoteMaster.action="QMSQuoteController";
          document.getElementById("QuoteMaster").submit();

		   }

	}

function showAutoCompleteAjaxforCustomer(obj,input)
	{
         var cust_id = obj.value.toUpperCase();
		 var targetName = input;
      		if(cust_id.length>=3)
		{
		  var url ='<%=request.getContextPath()%>/QMSQuoteAjaxController?type=AJAX&SearchOption=Cust_ID&Cust_id='+cust_id+'&dummytime='+((new Date()).getTime());
        showTableAutoComplete(url,input,targetName,obj);
		}
...
<body onLoad="initialize();displaySpans();populateCAP();testShipMode(0);changeConsoleVisibility('shipper');changeConsoleVisibility('consignee');salesPersonFlag();getClockTime();">
<form  method="post" name="QuoteMaster"  id ="QuoteMaster" >
...
%>
		<tr class="formdata">
			<td  width='15%' valign="top">Quote&nbsp;Type:&nbsp;<font color="#ff0000">*</font><br>
				...
				<select name="shipmentMode" size="1" class="select" onchange='testShipMode("<%=masterDOB.getShipmentMode()%>");clearListValues();displaySpans(),setWeightBreak()'>
...
%>
			...
			<td width ='15%' valign="top">Created&nbsp;Date:<br>
				<input class="text" maxLength="25" name="createdDate"  size="10" value="<%=currentDate%>" onBlur="changeToUpper(this);return dtCheck(this)" readonly>
			...
			<td width ='15%' valign="top">Eff&nbsp;Date:<font color="#ff0000">*</font><br>
				<input class="text" maxLength="25" name="effDate"  size="10" value="<%=currentDate%>" onBlur="extend(this)">
	   ...
	   <table width="101%"" cellpadding="4" cellspacing="1" bgcolor='#FFFFFF'>
		<tr class="formdata" >
			<td  width ='20%' rowspan='1' valign='top'>Customer&nbsp;Id :&nbsp;<font color="#ff0000">*</font>
				...
				<input class="text" maxLength="25" name="customerId"   id="customerId"  size="12" value="<%=masterDOB.getCustomerId()!=null?masterDOB.getCustomerId():""%>" row="0" onBlur="changeToUpper(this);chrnum(this);getCustAddressByAjax(this);getSalesPersonByAjax(this);getAttention(this);" onkeyup="showAutoCompleteAjaxforCustomer(this,'customerId');"<%=readOnly%> >	 <!--  Modified by Rakesh -->
				...
				<div id="menu-popup" style="position: absolute; visibility:hidden;Z-INDEX: 500; background-color:white " onclick='dontClearAutoComplete()'>
					<select  size="15" style='width:auto;height:auto;outline-style:auto'  id='names' selected='0' class='select' align='center'  onClick='setIdAutoComplete();'  onKeyUp='keyOnSelectAutoComplete(event.keyCode);'>
					</select>  
					</div>
...
%>
		...
		<td width='28%'>
		<!--@@ Modified by kiran.v on  17/11/2011 for Wpbn Issue 280269-->
	...
	<table width="101%"" cellpadding="4" cellspacing="1" bgcolor='#FFFFFF'>
		...
		<tr class="text">
			...
			<td  align="right">
				...
				<input class="input" name="button" id='SaveExit' type="button" value="Save & Exit" onclick="setSubmit(this);">
				<input class="input" name="button" type="button" value="Next >>" onclick="setSubmit(this);">
				<input type="hidden" name="submitt1">
